...
class CRMSetupElement extends HTMLElement {
    ...
    isExportOnly(){  
       return this.querySelector('.crm-setup-export-only').checked 
    }
    ...
    setData(input_distances_table, input_well_rates, producer_pressure) {
        ...
        this.producer_pressure = producer_pressure;
    ...
    buildRateTraces(rateBlock, visibleNames) {
        const traces = [];

        if (!rateBlock || !rateBlock.data || !rateBlock.dates) {
            return traces;
        }

        for (const well in rateBlock.data) {
            traces.push({
                x: rateBlock.dates,
                y: rateBlock.data[well],
                mode: 'lines',
                name: well,
                type: 'scattergl',
                line: { width: 1 },
                visible: visibleNames.includes(well)
            });
        }
    ...
    plotRates() {
        ...
        this.has_bhp = this.producer_pressure !== undefined &&
                       this.producer_pressure !== null;
        let keys = [
            'water_injection',
            'liquid_production',
            'oil_production',
        ...
        const extraRateKeys = Object.keys(well_rates)
            .filter(key => key !== 'dates')
            .filter(key => !keys.includes(key));

        const ProductionRatios = extraRateKeys.length >= 2;
        ...
        if (ProductionRatios) {
            const key5 = extraRateKeys[0];
            const key6 = extraRateKeys[1];

            traces5 = this.buildRateTraces(well_rates[key5], visibleProducers);
            traces6 = this.buildRateTraces(well_rates[key6], visibleProducers);

            layout4 = makeLayout(formatString(key5));
            layout5 = makeLayout(formatString(key6));
        }  
        
        if (this.has_bhp) {

            const pressureData = this.producer_pressure.data;
            const pressureDates = this.producer_pressure.dates;

            for (const well in pressureData) {

                const visibility = visibleProducers.includes(well);
        ...
        let row3 = this.querySelector('.chart-container3');
        let div31 = this.querySelector('#dd31');
        let div32 = this.querySelector('#dd32');

        if (ProductionRatios) {
            row3.classList.remove('hidden');
            div31.innerHTML = '';
            div32.innerHTML = '';
            div31.style.minHeight = '300px';
            div32.style.minHeight = '300px';
        } else {
        ...
        async function plotAllAndSync(element_this) {
            ...
            if (ProductionRatios) {
                await PlotlyNewPlot(div31, traces5, layout4, { responsive: true });
                await PlotlyNewPlot(div32, traces6, layout5, { responsive: true });
                divs.push(div31, div32);
            }
...
producerDivs.forEach((sourceDiv) => {
    ...
    sourceDiv.on('plotly_restyle', async (data) => {
        ...
        try {
            ...
            const changedVisibilityByName = new Map();

            traceIndices.forEach((traceIndex, i) => {
                const trace = sourceDiv.data[traceIndex];
                if (!trace) return;

                const visibility = Array.isArray(visibilityChange)
                    ? visibilityChange[i]
                    : visibilityChange;

                changedVisibilityByName.set(trace.name, visibility);
            });

            const otherProducerDivs = producerDivs.filter(div => div !== sourceDiv);

            const updatePromises = otherProducerDivs.map(targetDiv => {

                const targetIndices = [];
                const targetVisibility = [];

                targetDiv.data.forEach((trace, traceIndex) => {
                    if (changedVisibilityByName.has(trace.name)) {
                        targetIndices.push(traceIndex);
                        targetVisibility.push(changedVisibilityByName.get(trace.name));
                    }
                });
    ...
    displayDatesFromTrackbarInCharts(which){
        ...
        if (this_object.querySelector('.chart-container3')?.classList.contains('hidden') === false) {
            ids.push('#dd31', '#dd32');
        }  
        ...
        return `
            <div id='modelling-page' class='page'>
                ...
                <div class="chart-container2" style='display:flex; justify-content: space-evenly;'>
                 ...
                 <!-- Third row of side-by-side charts -->
                <div class="chart-container3 hidden" style='display:flex; justify-content: space-evenly;'>
                    <div id='dd31' style="width:49%"></div>
                    <div id='dd32' style="width:50%"></div>
                </div>
            ...
            <div class='card' id='liquid-history-match-card'>    
                ...
                <p></p>
                <h5>Model type:</h5>
                <select name="balance" class="form-select model-type" onchange="function sxx(this)
                    {  
                        //console.log("Changed model to" );
                        //console.log(this.value);
                        return; 
                        let model = this.value;
                        let disabled = (model=='crmtank') || (model=='crmip')
